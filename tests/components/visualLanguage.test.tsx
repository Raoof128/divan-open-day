...
it('uses safe hidden original geometry and culturally distinct poet portals', async () => {
  ...
  const rumi = screen.getByRole('button', { name: /A Moment of Reflection.*Rumi/u });
  ...
  expect(hafez.querySelector('[data-motif="pomegranate-cypress"]')).not.toBeNull();
  expect(rumi.querySelector('[data-motif="reed-rosette"]')).not.toBeNull();

  const decorativeSvgs = [...container.querySelectorAll('svg[data-divan-geometry]')];
  ...
  for (const svg of decorativeSvgs) {
    ...
    expect([...svg.attributes].some((attribute) => /^on/u.test(attribute.name))).toBe(false);
...
it('registers every repository-authored motif without a third-party artwork claim', async () => {
  ...
  expect(register).toContain('No runtime font request leaves the built origin.');
});
